...
//    private static final Logger LOGGER = Logger.getLogger(IdpLogger.class.getName());
     ...
     * @see NamedStatement
     */
    public EsitiLog scriviLog(LogDto logDto, Connection con)
            throws UnknownHostException, SQLException {
        EsitiLog retval = EsitiLog.UTENTE_OK;
        if (idpConfigLog.getQryRegistraEventoUtente() != null
                && !idpConfigLog.getQryRegistraEventoUtente().isEmpty()) {
...
//                        );
                ...
                String descrizione = logDto.getNmAttore()
                        + " - " + logDto.getDsEvento()
                        + descEstesa;
